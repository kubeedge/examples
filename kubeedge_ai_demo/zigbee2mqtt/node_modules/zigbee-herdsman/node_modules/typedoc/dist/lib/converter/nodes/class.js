            ...
            if (node.members) {
                node.members.forEach((member) => {
                    ...
                    if (child && member.name && ts.isPrivateIdentifier(member.name)) {
                        child.flags.setFlag(index_1.ReflectionFlag.Private, true);
                    }
                });
            }
            ...
            if (extendsClause) {
                ...
                if (type) {
                    ...
                    for (const typeToInheritFrom of typesToInheritFrom) {
                        typeParams = typeParams.concat(types_1.getTypeParametersOfType(typeToInheritFrom));
                    }
                    const typeArguments = typeParams.length > 0
                        ? types_1.getTypeArgumentsWithDefaults(typeParams, baseType.typeArguments)
                        : undefined;
