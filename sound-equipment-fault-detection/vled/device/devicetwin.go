...
const (
	observedDesiredValueOff string = "0"
	observedDesiredValueOn  string = "1"
)

var (
	observedDesiredValue string = observedDesiredValueOff
	mu                   sync.Mutex
)
...
func SetObservedDesiredValue(Value string) {
	mu.Lock()
	defer mu.Unlock()
	if Value == observedDesiredValueOn || Value == observedDesiredValueOff {
		observedDesiredValue = Value
	} else {
		klog.Infoln("Invalid Value")
	}
}
...
func GetObservedDesiredValue() string {
	mu.Lock()
	defer mu.Unlock()
	return observedDesiredValue
}

func (td *TwinData) SetObservedDesiredValue() {
	td.ObservedDesired.Value = observedDesiredValue
}
...
func (td *TwinData) GetPayLoad() ([]byte, error) {
	...
	// SetObservedDesired
	td.SetObservedDesiredValue()
	// SetDeviceData Hardware related
	err = td.Client.SetDeviceData(td.ObservedDesired.Value, td.VisitorConfig)
	if err != nil {
		return nil, fmt.Errorf("set device data failed: %v", err)
	}
